...
df_event <- read.csv('https://dqlab-dataset.s3-ap-southeast-1.amazonaws.com/event.csv', 
                     stringsAsFactors = F)
...
df_marketplace <- df_event %>% 
  ...
  select(loan_id, marketplace = created_at) # Mengganti nama "created_at"
...
#--- Berapa lama waktu yang dibutuhkan investor untuk pesan sejak pertama 
    #melihat detail loan ---#
